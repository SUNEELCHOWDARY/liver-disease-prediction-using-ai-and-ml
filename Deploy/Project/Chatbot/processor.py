...
model = load_model('Chatbot/chatbot_model(G).h5',compile=False)
import json
import random
...
words = pickle.load(open('Chatbot/words(G).pkl','rb'))
classes = pickle.load(open('Chatbot/classes(G).pkl','rb'))
...
def getResponse(ints, intents_json, source_lang='en', target_lang='en', save_path="C:/Users/abhi/Desktop/Final_Year_Project/ITPML35-FINAL/ITPML35 - FINAL CODING/Deploy/Project/Responses/response.mp3"):
    tag = ints[0]['intent']
    list_of_intents = intents_json['intents']
    translator = Translator()
    for i in list_of_intents:
